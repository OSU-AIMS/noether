#include <noether_tpp/tool_path_planners/raster/plane_variable_slicer_raster_planner.h>
...
namespace noether
{
PlaneVariableSlicerRasterPlanner::PlaneVariableSlicerRasterPlanner(DirectionGenerator::ConstPtr dir_gen,
                                                   OriginGenerator::ConstPtr origin_gen)
  : RasterPlanner(std::move(dir_gen), std::move(origin_gen))
{
}

void PlaneVariableSlicerRasterPlanner::setMinSegmentSize(const double min_segment_size)
{
  min_segment_size_ = min_segment_size;
}

void PlaneVariableSlicerRasterPlanner::setSearchRadius(const double search_radius) { search_radius_ = search_radius; }

void PlaneVariableSlicerRasterPlanner::generateRastersBidirectionally(const bool bidirectional)
{
  bidirectional_ = bidirectional;
}

ToolPaths PlaneVariableSlicerRasterPlanner::planImpl(const pcl::PolygonMesh& mesh) const
{
  if (!hasNormals(mesh))
  ...
  for (std::size_t i = 0; i < num_planes + 1; i++)
  {
    ...
    Eigen::Vector3d current_loc = start_loc + distance_so_far * cut_normal;
    distance_so_far += line_spacing_varied[i];
...
ToolPathPlanner::ConstPtr PlaneVariableSlicerRasterPlannerFactory::create() const
{
  auto planner = std::make_unique<PlaneVariableSlicerRasterPlanner>(direction_gen(), origin_gen());
  planner->setLineSpacing(line_spacing_varied);
